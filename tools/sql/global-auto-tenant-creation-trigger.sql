-- TRIGGER GLOBAL POUR AUTO-CRÉATION TENANT COMPLET
-- Se déclenche automatiquement à chaque création d'utilisateur dans auth.users
-- Suit l'ordre exact spécifié : invitation → tenant → profil → rôle → permissions → user_roles → employé → mise à jour invitation
...
-- 2. SUPPRIMER TOUS LES TRIGGERS EXISTANTS (y compris le nouveau)
...
-- 3. CRÉER LE NOUVEAU TRIGGER GLOBAL
...
BEGIN
  ...
  LOOP
    BEGIN
      -- Simuler l'insertion pour déclencher la logique de création
...
SELECT '🚀 TRIGGER GLOBAL INSTALLÉ' as status,
       'Tous les nouveaux utilisateurs auront automatiquement leur tenant créé' as description
UNION ALL
SELECT '🔧 RÉPARATION DISPONIBLE' as status,
       'Exécutez: SELECT repair_all_existing_users(); pour réparer les utilisateurs existants' as description;
